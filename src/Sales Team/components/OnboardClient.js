...
const OnboardClientModal = ({ open, onClose, lead }) => {
  ...
  useEffect(() => {
    if (lead) {
      const hasProjectDetails = lead.projectTitle || lead.projectType || lead.projectDescription;
      ...
      setFormData({
        ...
        email: lead.email || '',
        phone: lead.phone || '',
        projectTitle: hasProjectDetails ? lead.projectTitle : `${lead.company || 'Project'} ${lead.service || 'Engagement'}`,
        projectType: hasProjectDetails ? lead.projectType : lead.service || '',
        projectDescription: hasProjectDetails ? lead.projectDescription : `New ${lead.service || 'project'} engagement for ${lead.company || 'client'}`,
        startDate: hasProjectDetails ? lead.projectStartDate : lead.expectedCloseDate || new Date().toISOString().split('T')[0],
        endDate: hasProjectDetails ? lead.projectEndDate : "",
        duration: hasProjectDetails ? lead.projectDuration : "1 month",
        budget: lead.budget || '',
        notes: lead.notes || '',
        communicationChannel: "",
        sendWelcomeEmail: true,
  ...
  return (
    ...
    >
      <DialogTitle sx={{ 
        ...
        {hasProjectDetails && (
          <Chip 
            label="Project Details Provided" 
            size="small" 
            color="info"
            sx={{ ml: 2 }}
          />
            ...
            {hasProjectDetails && (
              <Paper elevation={0} sx={{ 
                ...
                <Typography variant="body2" sx={{ color: '#0369a1', mb: 1 }}>
                  The following project details were provided when this lead was created:
                </Typography>
                <Grid container spacing={1}>
                  {lead.projectTitle && (
                    <Grid item xs={6} md={4}>
                      <Typography variant="caption" sx={{ color: '#64748b' }}>Project Title:</Typography>
                      <Typography variant="body2">{lead.projectTitle}</Typography>
                    </Grid>
                  )}
                  {lead.projectType && (
                    <Grid item xs={6} md={4}>
                      <Typography variant="caption" sx={{ color: '#64748b' }}>Project Type:</Typography>
                      <Typography variant="body2">{lead.projectType}</Typography>
                    </Grid>
                  )}
                  {lead.projectStartDate && (
                    <Grid item xs={6} md={4}>
                      <Typography variant="caption" sx={{ color: '#64748b' }}>Start Date:</Typography>
                      <Typography variant="body2">{lead.projectStartDate}</Typography>
                    </Grid>
                  )}
                  {lead.projectDuration && (
                    <Grid item xs={6} md={4}>
                      <Typography variant="caption" sx={{ color: '#64748b' }}>Duration:</Typography>
                      <Typography variant="body2">{lead.projectDuration}</Typography>
                    </Grid>
                  )}
                  {lead.budget && (
                    <Grid item xs={6} md={4}>
