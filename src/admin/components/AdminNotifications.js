...
const NotificationItem = ({ notification, markAsRead, markAsUnread, archiveNotification }) => {
  ...
  return (
    <motion.div
      ...
      className={`p-4 border-b border-gray-100 ${!notification.viewed ? 'bg-blue-50' : 'bg-white'} cursor-pointer`}
...
const AdminNotifications = () => {
  ...
  const archiveNotification = async (id) => {
    try {
      // In a real implementation, you might want to move to an archive collection
      // or just filter out from the UI as shown here
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const batch = unreadNotifications.map(n => 
        updateDoc(doc(db, 'admin-notifications', n.id), { viewed: true })
      );
  ...
  const archiveAll = async () => {
    try {
      // In a real implementation, you would archive all visible notifications
      ...
      <Box className="max-w-6xl mx-auto px-4 py-8" sx={{marginTop:'60px'}}>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {['lead', 'alert', 'system', 'payment'].map(type => (
                <Chip
                  key={type}
                  label={type}
                  clickable
