    ...
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
            ...
            {isHovered && (
              <div className="flex gap-2">
                {notification.read ? (
                  ...
                  >
                    <FiClock className="text-gray-500 hover:text-blue-600" size={16} />
                  </IconButton>
                ) : (
                  <IconButton 
                    onClick={(e) => {
                    ...
                    title="Mark as read"
                  >
                    <FiCheck className="text-gray-500 hover:text-green-600" size={16} />
                  </IconButton>
                )}
                <IconButton 
                  onClick={(e) => {
                  ...
                  title="Archive"
                >
                  <FiX className="text-gray-500 hover:text-red-600" size={16} />
                </IconButton>
              </div>
            )}
          </div>
          ...
          {notification.submissionTitle && (
            <Typography variant="body2" className="text-gray-600 mt-1">
              Submission: {notification.submissionTitle}
            </Typography>
          )}
...
const ClientNotifications = () => {
  ...
  const markAsRead = async (id) => {
    try {
      await updateDoc(doc(db, 'client-notifications', id), {
        read: true
      });
  ...
  const archiveNotification = async (id) => {
    try {
      // In production, you might want to move to an archive collection
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const batch = unreadNotifications.map(n => 
        updateDoc(doc(db, 'client-notifications', n.id), { read: true })
      );
  ...
  const archiveAll = async () => {
    try {
      // In production, you would archive all visible notifications
  ...
  const filteredNotifications = notifications.filter(notification => {
    ...
    if (activeTab === 'approvals' && notification.type !== 'approval') return false;
    if (activeTab === 'alerts' && notification.type !== 'alert') return false;
    
    // Search filtering
    if (searchQuery && 
        !notification.message.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !(notification.projectTitle && notification.projectTitle.toLowerCase().includes(searchQuery.toLowerCase()))) {
      return false;
    }
    ...
    // Status filtering
    if (selectedFilters.status === 'read' && !notification.read) return false;
    if (selectedFilters.status === 'unread' && notification.read) return false;
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {['approval', 'alert', 'update'].map(type => (
                <Chip
                  key={type}
                  label={type}
                  clickable
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
          ...
          >
            ...
            <StyledTab 
              ...
              iconPosition="end"
            />
            <StyledTab label="Approvals" value="approvals" />
