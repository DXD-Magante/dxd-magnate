...
const MainContent = ({ activeSection, activeSubSection, currentUser }) => {
    ...
    const fetchProjects = async () => {
      try {
        ...
        const projectsData = querySnapshot.docs
          ...
          .filter(project => project.projectManagerId === currentUser.uid);
  ...
  return (
    <Box component="main" sx={{ flexGrow: 1, p: 3, backgroundColor: '#f8fafc' }}>
      ...
      {activeSection === "Dashboard" ? (
        ...
        <DeadlinesView />
      ) :  activeSubSection === "Resource Allocation" ? (
        <ResourceAllocation />
      ) :  activeSubSection === "Project Files" ? (
        <ProjectFiles />
      ) : activeSubSection === "Communication" ? (
        <Communication />
      ) : activeSubSection === "Project Reports" ? (
        <ProjectReports/>
