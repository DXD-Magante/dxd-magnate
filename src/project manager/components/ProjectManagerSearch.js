...
// Styled components with modern professional look
...
const ProjectManagerSearch = ({ assignedProjects, teamMembers }) => {
                  ...
                  >
                    <ListItemAvatar sx={{ minWidth: '44px' }}>
                      ...
                      >
                        ...
                        >
                          {result.type === 'project' ? 
                            result.data.title.charAt(0).toUpperCase() : 
                    ...
                    <ListItemText
                      primary={
                        <Typography variant="body2" fontWeight={500} noWrap>
                          {highlightMatch(
                            result.type === 'project' ? result.data.title : result.data.name,
                            searchQuery
                          )}
                        </Typography>
                      }
                      secondary={
                        <>
                          ...
                          >
                            {result.type === 'project' ? 
                              highlightMatch(`Client: ${result.data.clientName}`, searchQuery) : 
                              highlightMatch(`Role: ${result.data.projectRole || 'Team Member'}`, searchQuery)}
                          </Typography>
                          {result.type === 'project' && (
                            <Chip
                              ...
                              sx={{
                                ...
                                backgroundColor: 
                                  ...
                                  result.data.status === 'In progress' ? 'info.light' :
                                  result.data.status === 'On hold' ? 'error.light' : 'grey.100',
                                color: 
                                  result.data.status === 'Completed' ? 'success.dark' :
                                  result.data.status === 'In progress' ? 'info.dark' :
                                  result.data.status === 'On hold' ? 'error.dark' : 'grey.800',
                              }}
                            />
                          )}
                        </>
