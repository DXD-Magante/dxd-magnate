    ...
    const fetchPerformanceData = async () => {
      ...
      try {
        ...
        if (selectedProject.teamMembers && selectedProject.teamMembers.length > 0) {
          ...
          setPerformanceData(memberPerformance);
        }
      } catch (error) {
      ...
      <Box className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <Box>
          ...
          <Typography variant="body2" sx={{ color: '#64748b' }}>
            Track and analyze your team's productivity and task completion
          </Typography>
        </Box>
        ...
        <>
              ...
              <PerformanceCard>
                ...
                <Typography variant="h4" sx={{ fontWeight: 'bold', mb: 1 }}>
                  {performanceData.length > 0 ? 
                    Math.round(performanceData.reduce((sum, member) => sum + member.efficiency, 0)) / performanceData.length : 
                    0}%
                </Typography>
                <Typography variant="caption" sx={{ color: '#64748b' }}>
                  Team average
          ...
          <Paper sx={{ p: 3, borderRadius: 2, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
            <Box className="flex items-center justify-between mb-4">
              ...
              <Chip 
                label={`${performanceData.length} team members`} 
                size="small" 
                        ...
                        <Box className="flex items-start space-x-3">
                          <Avatar 
                            ...
                            sx={{ 
                              width: 48, 
                              height: 48,
                              bgcolor: member.avatar ? 'transparent' : '#E0E7FF',
                              color: member.avatar ? 'inherit' : '#4F46E5',
                              fontWeight: 'bold'
                            }}
                          >
